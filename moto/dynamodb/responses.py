...
class DynamoHandler(BaseResponse):
    ...
    def create_table(self):
        ...
        table = dynamodb_backend.create_table(table_name,
                                               schema=key_schema,
    ...
    def update_table(self):
        ...
        if 'GlobalSecondaryIndexUpdates' in self.body:
            table = dynamodb_backend.update_table_global_indexes(
                name, self.body['GlobalSecondaryIndexUpdates'])
        if 'ProvisionedThroughput' in self.body:
            throughput = self.body["ProvisionedThroughput"]
            table = dynamodb_backend.update_table_throughput(name, throughput)
        return dynamo_json_dump(table.describe())
    ...
    def put_item(self):
        ...
        try:
            result = dynamodb_backend.put_item(
        ...
        for table_name, table_requests in table_batches.items():
            for table_request in table_requests:
                ...
                if request_type == 'PutRequest':
                    ...
                    keys = request['Key']
                    item = dynamodb_backend.delete_item(table_name, keys)
    ...
    def get_item(self):
        ...
        key = self.body['Key']
        try:
            item = dynamodb_backend.get_item(name, key)
        except ValueError:
    ...
    def batch_get_item(self):
        ...
        for table_name, table_request in table_batches.items():
            ...
            for key in keys:
                item = dynamodb_backend.get_item(table_name, key)
                if item:
    ...
    def query(self):
        ...
        if key_condition_expression:
            ...
            table = dynamodb_backend.get_table(name)
            ...
            if key_conditions:
                hash_key_name, range_key_name = dynamodb_backend.get_table_keys_name(
        ...
        items, scanned_count, last_evaluated_key = dynamodb_backend.query(
    ...
    def scan(self):
        ...
        items, scanned_count, last_evaluated_key = dynamodb_backend.scan(name, filters,
                                                                          limit,
                                                                          exclusive_start_key)
    ...
    def delete_item(self):
        ...
        item = dynamodb_backend.delete_item(name, keys)
        if item:
    ...
    def update_item(self):
        ...
        existing_item = dynamodb_backend.get_item(name, key)
        ...
        item = dynamodb_backend.update_item(
